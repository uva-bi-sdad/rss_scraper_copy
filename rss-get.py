...
class RSSer():
    ...
    def search_keyword(self, keyword, date_start=None, date_end=None):
        ...
        if query is not None:
            return requests.get(query, headers={'Accept': 'application/json'})
        else:
            return None
...
def main(args):
    ...
    for source in args.sources:
        ...
        for keyword in tqdm(args.keywords):
            ...
            r = rss.search_keyword(keyword, start_date_str, end_date_str)
            logging.debug(r)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DSPG pull from Google RSS')
    parser.add_argument('-k', '--keywords', nargs='+',
                        help='Keywords to parse', required=True)
